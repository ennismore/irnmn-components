...
class IrnmnRoomCard extends HTMLElement {
    ...
    /**
     * Retrieves the content of the "room-pricing" slot template, if present.
     */
    getPricingSlotContent() {
        if (this.roomPricingTemplate) return;
        const template = this.querySelector('template[slot="room-pricing"]');
        this.roomPricingTemplate = template ? template.innerHTML.trim() : null;

        console.log(
            'room-pricing slot content:',
            this.roomPricingTemplate || 'none',
        );
    }
     ...
     */
    renderPricing() {
        // If a custom pricing template is provided, use it
        if (this.roomPricingTemplate) {
            return this.roomPricingTemplate;
        }
        // Otherwise, render a default pricing button
        return `
