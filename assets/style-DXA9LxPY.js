var g=Object.defineProperty;var v=(d,s,e)=>s in d?g(d,s,{enumerable:!0,configurable:!0,writable:!0,value:e}):d[s]=e;var h=(d,s,e)=>v(d,typeof s!="symbol"?s+"":s,e);class b extends HTMLElement{static get observedAttributes(){return["title","description","badge-label","images","extras","room-amenities","hotel-amenities","link-360","arrow-svg","labels"]}constructor(){super(),this.uniqueId=`card-${Math.random().toString(36).substr(2,9)}`}get roomCode(){return this.getAttribute("room-code")||""}get title(){return this.getAttribute("title")||""}get description(){return this.getAttribute("description")||""}get images(){try{return JSON.parse(this.getAttribute("images")||"[]")}catch{return[]}}get badgeLabel(){return this.getAttribute("badge-label")||""}get extras(){try{return JSON.parse(this.getAttribute("extras")||"[]")}catch{return[]}}get roomAmenities(){try{return JSON.parse(this.getAttribute("room-amenities")||"[]")}catch{return[]}}get hotelAmenities(){try{return JSON.parse(this.getAttribute("hotel-amenities")||"[]")}catch{return[]}}get link360(){return this.getAttribute("link-360")||""}get labels(){try{return JSON.parse(this.getAttribute("labels")||"{}")}catch{return{}}}get arrowSvg(){return this.getAttribute("arrow-svg")||`
        ...
        `}renderExtras(s=!0){return`
            <div class="room-card__extras">
                ...
                ${s?`<button aria-label="${this.labels.more?`${this.title} ${this.labels.more}`:`${this.title} More info`}" class="btn btn-secondary expand-room-modal">${this.labels.more||"More info"}</button>`:""}
            </div>
