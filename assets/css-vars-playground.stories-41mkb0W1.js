import{x as k}from"./iframe-DbjO1NWJ.js";import"./style-DRSgatFx.js";const z=`:root,
body {
    /* => Spacing scale */
    --space-1: 4px;
    --space-2: 8px;
...
`);for(const r of R){if(r.includes("end-parsing"))break;if(!r.trim())continue;const o=r.match(/\/\*\s*=>\s*(.+?)\s*\*\//);o&&(h=o[1].trim());const a=r.match(/--([a-zA-Z0-9-_]+)\s*:\s*(.+?);/);if(a){const e=`--${a[1]}`,i=a[2].trim();let t=!1,n="";const d=i.match(/^var\((--[a-zA-Z0-9-_]+)\)$/);d&&(t=!0,n=d[1]),f[e]={name:e,rawValue:i,isReference:t,referenceTarget:n,category:h}}}const v=(r,o=new Set)=>{if(o.has(r))return{};o.add(r);const a=f[r];return a?a.isReference?v(a.referenceTarget,o):a:{}},c={...$},u=Object.keys(f);for(const r of u){const o=f[r],a=o.isReference?v(o.referenceTarget):{};let e="text",i=o.rawValue,t="";const n=o.isReference?a.rawValue:o.rawValue;/^#[0-9a-f]{3,6}$/i.test(n)||/^rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+(?:\s*,\s*(?:\d*\.?\d+)\s*)?\)$/i.test(n)?e="color":/^\d*\.?\d+px$/.test(n)?(i=parseFloat(n),e={type:"range",min:0,max:i>100?1e3:100},t="px"):/^\d*\.?\d+rem$/.test(n)?(i=parseFloat(n),e={type:"range",min:0,max:5,step:.125},t="rem"):/^\d*\.?\d+%$/.test(n)&&(i=parseFloat(n),e={type:"range",min:0,max:100},t="%");const d=r.replace("--","").replace("room-card","").replace(/-/g," ")+(t?` (${t})`:""),l=u.filter(p=>{if(p===r)return!1;const s=v(p);if(!s)return!1;const m=s.rawValue;return e==="color"?/^#[0-9a-f]{3,6}$/i.test(m):!!(t&&m.endsWith(t)||!t&&!/^#[0-9a-f]{3,6}$/i.test(m)&&!/px|rem|%/.test(m))});c[`__switch__${r}`]={name:`${d} Mode`,control:{type:"inline-radio"},options:["Custom","Reference"],defaultValue:o.isReference?"Reference":"Custom",table:{category:o.category}},c[r]={name:` 🟢 ${d}`,control:e,defaultValue:o.isReference?void 0:i,unit:t,table:{category:o.category},if:{arg:`__switch__${r}`,eq:"Custom"}},c[`__ref__${r}`]={name:` 🔗 ${d}`,control:{type:"select"},options:e!="text"?l:u,defaultValue:o.isReference?o.referenceTarget:l[0],table:{category:o.category},if:{arg:`__switch__${r}`,eq:"Reference"}}}const T=r=>{var o;for(const a of u){const e=r[`__switch__${a}`],i=r[`__ref__${a}`],t=r[a],n=((o=c[a])==null?void 0:o.unit)??"",d=e==="Reference"?`var(${i})`:`${t}${n}`;document.documentElement.style.setProperty(a,d),document.body.style.setProperty(a,d)}},N={title:"Playground/Room Card",argTypes:c},V=r=>{T(r);const o=L(r),a=Object.entries(o).map(([e,i])=>`${e}: ${i};`).join(`
